...
<vxml version="2.1" revision="4" 
  ...
  <form id="readIWBankFDIC">
    <block>
      <assign name="currentMenu" expr="'readIWBankFDIC'" />
      <log label="com.tellme.proprietary.dreyfusgama.getbankfdic.task.success" />

      <prompt bargein="true">
        <%
          ArrayList<FundYieldInfoDTO> bankFDICRates = (ArrayList<FundYieldInfoDTO>) session.getAttribute("bankFDICRates");
          if (bankFDICRates != null) {
            for (FundYieldInfoDTO item : bankFDICRates) {
        ...
        <foreach item="element" array="percentArray">
          <audio expr="standardAudioUrl + element + '.wav'">
            <voice name="tom"><value expr="element" /></voice>
          </audio>
        </foreach>
      ...
      <goto next="<%= Encode.forHtmlAttribute(mutualSSLDto.getMutualSslUrl()) %>/dreyfusgama/getIWbankFDIC.jsp" />
    </block>
  </form>
